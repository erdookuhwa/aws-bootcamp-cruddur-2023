...
- I used `cdk destroy` to tear down resources from the stack; then I created my assets S3 bucket outside of the stack so I can manage the resource separately from the rest of the stack. I achieved that using:
  ...
  ```
- After creating this bucket out of the stack, I imported the bucket to be viewable in the stack but not manageable by it.
...
- I installed **_Thunder Client_** from Market place to test the presigned link generated by [`function.rb`](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/aws/lambdas/cruddur-upload-avatar/function.rb)
  -![image](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/_docs/assets/Week8_testAPIgw.png)
...
- **_Creating the API Gateway_**
  ...
  - I accepted default for _stages_, _configured the routes_ to point to the bucket folder: _`/avatars/key_upload`_, etc
    ...
    - ![image](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/_docs/assets/Week8_proxy.png)
...
- In [`ProfileForm.js`](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/ae4a0157355f23c9ddc1670a0c4f9a6d8d6157ea/frontend-react-js/src/components/ProfileForm.js), I have added the function to make a call to the presigned URL & upload avatar
...
- Successfully generated the **Presigned URL**
  - ![image](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/_docs/assets/Week8_presignedURL.png)

#### Creating the JWT Lambda Layer
Now that the Pre-signed URL is generated, the next step is to use my _uploads function_ to access my API Gateway URL, then the Lambda Authorizer verifies the JSON Web Token of the user, and if verified, uploads to the bucket is possible. To achieve this, I added a _Lambda Layer_ to my _uploads function_ which had the Gems required for the Ruby 2.7 runtime.
- I zipped the function in my GitPod
  - ![image](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/_docs/assets/Week8_zipRubyFxnGems.png)
  - ![image](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/_docs/assets/Week8_rubyLambdaZip.png)
- I uploaded my zipped file:
  - ![image](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/_docs/assets/Week8_uploadLambdaZip.png)
- Adding my lambda layer to the function
  - ![image](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/_docs/assets/Week8_addingLambdaLayer.png)

#### Fixing CORS
- I added a _CORS policy_ to my uploads-bucket to allow resource sharing
```json
  [
    {
        "AllowedHeaders": [
...
- In my _uploads function_, I added headers to prevent CORS issues on a browser
...
#### Testing
- In my GitPod env, I did `docker-compose up` and at my frontend url > Profile > I uploaded an avatar which posted to my S3 bucket in AWS
  - ![image](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/_docs/assets/Week8_uploadUsingSub.png)

#### Render Avatar from CloudFront
- In [`show.sql`](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/backend-flask/db/sql/users/show.sql), the user's cognito_user_uuid is one of the attributes being selected which is then passed down to the components
- [`ProfileAvatar.js`](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/frontend-react-js/src/components/ProfileAvatar.js): Passing down the user's uuid to props, I'm able to render the user's avatar
- I updated [`ProfileInfo.js`](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/frontend-react-js/src/components/ProfileInfo.js) to use the newly created _ProfileAvatar component_ to render the image to screen
##### Testing
At my app's FE URL/profile, I was able to view my user's new avatar!
 - ![image](https://github.com/erdookuhwa/aws-bootcamp-cruddur-2023/blob/main/_docs/assets/Week8_renderAvatarCF.png)
